...
/// Check that the Nixpkgs attribute values corresponding to the packages in the given by-name
/// subpath are of the form `callPackage <package_file> { ... }`. See the `./eval.nix` file for how
/// this is achieved on the Nix side.
///
...
/// Handles the evaluation result for an attribute _not_ in the given by-name subpath, turning it
...
fn handle_non_by_name_attribute(
    ...
    let package = match non_by_name_attribute {
        ...
        // migrated to the by-name subpath if it doesn't evaluate, since we need to check that it's
        // a derivation.
        //
        ...
        // fixing it requires a move to the by-name subpath, which could happen more often and
        ...
        EvalSuccess(AttributeInfo {
            ...
            attribute_variant:
                AttributeVariant::AttributeSet {
                    // As of today, non-derivation attribute sets can't be in the by-name subpath.
                    is_derivation: true,
                    strict_deps,
                    structured_attrs,
            ...
            // - Know that the attribute _could_ be migrated to the by-name subpath, which is
            //   `Loose`
                    ...
                    (true, Some(syntactic_call_package)) => {
                        ...
                        match syntactic_call_package.relative_path {
                            Some(ref rel_path) if rel_path.starts_with(by_name_subpath) => {
                                ...
                                //   }
                                //
                                // While such definitions could be moved to the by-name subpath
                                // by using `.override { someFlag = true; }` instead, this changes
                            ...
                            _ => Loose((
                                syntactic_call_package.clone(),
                                location.file.clone(),
                                by_name_subpath.into(),
                            )),
                            }
                        }
                    }
            ...
            ratchet::Package {
                ...
                // because they're not using the by-name subpath which would allow avoiding it. So
                // the ratchet stays `Tight` regardless of the other checks in this function.
